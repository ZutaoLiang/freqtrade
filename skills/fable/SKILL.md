...
### 3.1 当前资金费候选摘要(2026-09-03)

当前代码在 2025-02-01 ~ 2026-08-16、204 个有效永续、fee 0.001/边、真实 1h
funding/mark、1000 USDT 钱包、最多 6 仓的统一回测中:

- 显式 1m baseline:固定 150 USDT/仓,+87.90%,PF 1.150,最大回撤 19.98%,6/7 季度为正。
- **1m + 因果 15m 同向动量硬过滤**:1021 笔,+113.16%,PF 1.258,最大回撤 12.66%,
  7/7 季度为正,是当前最强候选。
- 保留未确认信号但给半仓只到 +91.25%;未确认半仓桶自身均值 -11.95bps、亏 20.86 USDT,
  还会占用 6 仓上限,所以硬过滤优于软降仓。
- 真正的 5m 信号加 `--timeframe-detail 1m` 只有 +65.81%,PF 1.111,最大回撤 33.52%。
  旧文档 §5.8 曾把实际元数据为 1m 的结果标成 5m;不要再引用旧标题做周期判断。
- 硬过滤用每仓约权益 15% 复利后,+173.33%,最大回撤 20.44%;固定金额是 +113.16% / 12.66%。
  复利提高绝对收益,但 PF 从 1.258 降到 1.204,收益/回撤从 8.94 降到 8.48。

完整定义、季度表、仓位范围与限制见 `funding-skew-momentum.md` §5.10。15m 过滤来自同一历史
样本上的探索,**尚不是未见样本证据**;生产配置在独立 OOS 或 dry-run 验证前不自动切换。
