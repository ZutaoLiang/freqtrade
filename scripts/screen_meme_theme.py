"""
Screen and evaluate strategies on Theme 3: Meme Tokens (10 tokens).
Tokens: DOGE, 1000SHIB, 1000PEPE, WIF, 1000BONK, 1000FLOKI, POPCAT, MOODENG, PNUT, FARTCOIN
...
STRATEGIES = [
    ("Dual Squeeze (Baseline 18h Exit)", "DualSqueezeBtcTrend1h", "user_data/minute_research/meme_theme/config_meme_1h.json"),
    ("Dual Squeeze + Vol Filter (> 1.3x)", "DualSqAi_V1_VolFilter", "user_data/minute_research/meme_theme/config_meme_1h.json"),
    ("Dual Squeeze + Vol + 6h Stagnation", "DualSqAi_V3_VolAndStagnation", "user_data/minute_research/meme_theme/config_meme_1h.json"),
    ("Conditional Time Exit (18h/36h) + Vol", "DualSqAi_ConditionalTimeExit_Vol", "user_data/minute_research/meme_theme/config_meme_1h.json"),
    ("Funding Exhaustion Short (R24 5m)", "FundingExhaustionShort5m", "user_data/minute_research/meme_theme/config_meme_5m.json"),
    ("AllWeather V2 Combined (5m)", "AllWeatherRegimeAdaptiveV2", "user_data/minute_research/meme_theme/config_meme_5m.json"),
    ("AllWeather V2 Flushout Dip-buy (5m)", "AllWeatherV2_Flushout", "user_data/minute_research/meme_theme/config_meme_5m.json"),
]
...
def main():
    print("=" * 80)
    print("SCREENING THEME 3: MEME TOKENS (10 TOKENS)")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("MEME TOKENS (10 TOKENS) SCREENING REPORT")
    print("=" * 80 + "\n")
    
    header = "| 策略名称 | TRAIN 收益 (PF, t, 笔数) | VALID-C 收益 (PF, t, 笔数) | HOLDOUT 收益 (PF, t, 笔数) | HOLDOUT 回撤 |"
    sep = "| :--- | :---: | :---: | :---: | :---: |"
    print(header)
    print(sep)
    
    for r in results:
        train_str = f"{r.get('TRAIN_profit', 0)}% (PF {r.get('TRAIN_pf', 0)}, t {r.get('TRAIN_t', 0)}, {r.get('TRAIN_trades', 0)}笔)"
        valid_str = f"{r.get('VALID-C_profit', 0)}% (PF {r.get('VALID-C_pf', 0)}, t {r.get('VALID-C_t', 0)}, {r.get('VALID-C_trades', 0)}笔)"
        hold_str = f"{r.get('HOLDOUT_profit', 0)}% (PF {r.get('HOLDOUT_pf', 0)}, t {r.get('HOLDOUT_t', 0)}, {r.get('HOLDOUT_trades', 0)}笔)"
        dd_str = f"{r.get('HOLDOUT_dd', 0)}%"
        print(f"| **{r['label']}** | {train_str} | {valid_str} | {hold_str} | {dd_str} |")
        
    out_path = "user_data/minute_research/meme_theme/meme_screening_results.json"
