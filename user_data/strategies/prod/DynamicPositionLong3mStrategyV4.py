...
class DynamicPositionLong3mStrategyV4(IStrategy):
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
        ...
        logger.info(f'Stake amount for {pair}={stake_amount:.4f}')
        return stake_amount
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if price_increase >= self.position_adjustment_pct:
            ...
            logger.info(f'Position addition #{count_of_entries+1} for {trade.pair} with amount {stake_to_increase:.5f} triggered at price increase: {price_increase:.2%}(current:{current_rate:.5f}/previous_rate:{latest_order.average:.5f}), profit:{current_profit:.2f} at {current_time}')
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        if not after_fill:
            ...
            # if current_profit > 0.05 * leverage:
        ...
        if current_profit > 0.32 * leverage:
            ...
            new_stoploss = 0.02 * leverage
        else:
            price_diff = current_rate / trade.open_rate - 1
            if price_diff >= 0.015:
                logger.info(f'Setting {pair} stoploss with low profit:{current_profit:.2%}, price_change:{price_diff:.2%}, current_rate:{current_rate:.5f}, open_rate:{trade.open_rate:.5f} at {current_time}')
                # 调仓（有浮盈才加仓）之后，保证最差不低于略高手续费的止损线
                new_stoploss = abs(price_diff - 0.01) * leverage
            else:
                # 很小的止损线，基本相当于立即止损退出
                new_stoploss = 0.005 * leverage

            # 低利润或者已经亏损的情况下，往下放两个点作为承受风险博取收益的止损价位
            # stoploss_add_pct = 0.02 * leverage
