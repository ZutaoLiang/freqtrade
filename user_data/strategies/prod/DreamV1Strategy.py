...
class DreamV1Strategy(IStrategy):
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        open_profit_abs = current_profit * trade.stake_amount
        ...
        logger.info(f'{trade.pair} total profit:{total_profit_abs:.4f}(open:{open_profit_abs:.4f}, close:{close_profit_abs:.4f}) at {current_time}')
        if total_profit_abs < profit_drawdown_threshold:
            exit_reason = f'Max loss'
            logger.info(f'{exit_reason} for {pair}:{total_profit_abs:.4f} < {profit_drawdown_threshold:.4f} at {current_time}')
            return exit_reason
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        reversion_stake = trade.stake_amount * price_change
        
        if abs(reversion_stake) > min_stake:
            # and abs(adjustment_value) < max_stake:
            logger.info(f'Mean reversion adjustment for {trade.pair} with stake amount:{reversion_stake:.4f}')
            trade.set_custom_data(self.REVERSION_PRICE, current_rate)
