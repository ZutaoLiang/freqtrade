...
class ChandelierExitStrategy(IStrategy):
    minimal_roi = {"0": 100}
    ...
    long_mult = DecimalParameter(1.5, 2.5, default=1.5, space='buy', optimize=True, decimals=1)
    
    ema_length = IntParameter(20, 500, default=base_period*12, space='buy')
    smooth_length = IntParameter(20, 500, default=base_period, space='buy')
    
    adx_length = IntParameter(10, 500, default=14, space='buy')
    ...
    def chandelier_exit(self, dataframe: DataFrame, length: int, mult: float, suffix: str = '') -> DataFrame:
        ...
        high = pta.ema(close=dataframe['high'], length=self.smooth_length.value)
        low = pta.ema(close=dataframe['low'], length=self.smooth_length.value)
        close = pta.ema(close=dataframe['close'], length=self.smooth_length.value)

        atr = pta.atr(high=high, low=low, close=close, length=length)
        
        long_stop = high.rolling(window=length).max() - mult * atr
        short_stop = low.rolling(window=length).min() + mult * atr
        
        dataframe[f'high_max_{suffix}'] = high.rolling(window=length).max() - mult * atr
        dataframe[f'low_min_{suffix}'] = low.rolling(window=length).min() + mult * atr
        ...
        dataframe[f'close_{suffix}'] = close
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Calculate Chandelier Exit with different lengths
        ...
        dataframe['ema'] = pta.ema(close=dataframe['close'], length=self.ema_length.value)

        # Calculate ADX using pandas_ta
        adx = pta.adx(high=dataframe['high'], low=dataframe['low'], close=dataframe['close'], length=self.adx_length.value)
        dataframe['adx'] = adx[f'ADX_{self.adx_length.value}']
        dataframe[f'adx_48'] = pta.adx(high=dataframe['high'], low=dataframe['low'], close=dataframe['close'], length=48)[f'ADX_48']
        dataframe[f'adx_72'] = pta.adx(high=dataframe['high'], low=dataframe['low'], close=dataframe['close'], length=72)[f'ADX_72']
