...
class TotalProfitDrawdownExit:
    def __init__(self, leverage):
        ...
        self.PROFIT_CHECK_THRESHOLD = 15
        self.PAIR_DRAWDOWN_THRESHOLD = 0.07
        self.TOTAL_DRAWDOWN_THRESHOLD = self.PAIR_DRAWDOWN_THRESHOLD * leverage
    ...
    def update_and_check_exit(self, pair: str, open_profit: float, min_rate: float, max_rate: float, current_rate: float, is_short: bool, leverage, current_time) -> bool:
        ...
        logger.info(f'Current total profit:{total_profit:.4f}, open:{self.open_total_profit:.4f}, closed:{self.closed_total_profit:.4f} at {current_time}')
        ...
        if drawdown >= self.TOTAL_DRAWDOWN_THRESHOLD:
            ...
            large_drawdown = (pair_drawdown >= self.PAIR_DRAWDOWN_THRESHOLD) and open_profit > 0
...
class DynamicPosition3mStrategyV11(IStrategy):
    ...
    initial_position_ratio = 1/trade_leverage
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        if count_of_entries < 4 and (0.01 * leverage) < current_profit and current_profit < (0.02 * leverage) and (current_time - timedelta(minutes=40)) > trade.open_date_utc:
        ...
        profit_drawdown_threshold = -2 * (total * leverage * self.initial_position_ratio * self.absolute_drawdown_profit_ratio) / self.max_open_trades
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        has_profit = current_profit > 0.001 * trade.leverage
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        if after_fill:
            if filled_count <= 1:
                ...
                new_stoploss = 0.6 * current_profit
                ...
                return new_stoploss
                # return stoploss_from_absolute(stop_rate=abs_rate, current_rate=current_rate, is_short=trade.is_short, leverage=trade.leverage)
            else:
                new_stoploss = 0.001 * leverage
                abs_rate = current_rate*(1 + (1 if trade.is_short else -1) * new_stoploss/trade.leverage)
                logger.info(f'Set {pair} negative profit stoploss rate:{abs_rate:.5f}({new_stoploss:.2%}) after fill relative to current_rate:{current_rate:.5f}, open_rate:{trade.open_rate:.5f}, current_profit:{current_profit:.2%} at {current_time}')
                return new_stoploss
        
        if current_profit > 0.25 * leverage:
            return 0.1 * leverage
        elif current_profit > 0.15 * leverage:
        ...
        atr_multiplier = 6
        profit_ratio = current_profit / leverage
        if profit_ratio > 0.1:
            atr_multiplier = 5
        
        if trade.is_short:
            stop_loss_price = trade.min_rate + (atr_multiplier * atr)
            if current_rate > stop_loss_price:
                logger.info(f'ATR stop loss: {pair} rate:{current_rate:.4f} greater than:{stop_loss_price:.4f} at {current_time}')
                return None
            return stoploss_from_absolute(stop_rate=stop_loss_price, current_rate=current_rate, is_short=trade.is_short,
                                          leverage=trade.leverage)
        else:
            stop_loss_price = trade.max_rate - (atr_multiplier * atr)
            if current_rate < stop_loss_price:
                logger.info(f'ATR stop loss: {pair} rate:{current_rate:.4f} lower than:{stop_loss_price:.4f} at {current_time}')
                return None
            return stoploss_from_absolute(stop_rate=stop_loss_price, current_rate=current_rate, is_short=trade.is_short,
                                          leverage=trade.leverage)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.is_long:
            ...
            ema_mid_up_mask = self.ema_up_n_days_mask(dataframe, 'ema_mid', self.ema_trend_length * 2)
            ...
            ema_mid_down_mask = self.ema_down_n_days_mask(dataframe, 'ema_mid', self.ema_trend_length * 2)
            ema_long_down_mask = self.ema_down_n_days_mask(dataframe, 'ema_long', self.ema_trend_length * 3)
