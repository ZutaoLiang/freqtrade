...
class DynamicPosition1mStrategyV2(IStrategy):
    ...
    def is_low_profit(self, current_profit: float, trade: Trade):
        return current_profit < self.position_adjustment_pct * self.get_leverage(trade)
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime, current_rate: float, current_profit: float, **kwargs,) -> Optional[Union[str, bool]]:
        if self.is_long_time(current_time, trade) and self.is_low_profit(current_profit, trade):
        ...
        if low_profit or large_drawback:
            ...
            if stake_to_decrease < -1e-4:
                if abs(stake_to_decrease) < abs(min_stake):
                    logger.info(f'Adjusting partial decrease stake {stake_to_decrease:.4f} to {-abs(min_stake):4f} according to min_stake for {trade.pair}')
            ...
            if filled_count > 2:
                stake_to_decrease = -(trade.amount * current_rate) / 2
                ...
                if low_profit:
                    return (stake_to_decrease, 'low profit exit')
                elif large_drawback:
                    return (stake_to_decrease, 'large drawback exit')
