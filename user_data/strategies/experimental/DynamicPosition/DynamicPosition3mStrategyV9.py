...
class TotalProfitDrawdownExit:
    def __init__(self):
        ...
        self.PAIR_DRAWDOWN_THRESHOLD = 0.035
    ...
    def update_and_check_exit(self, pair: str, current_profit: float, max_rate: float, current_rate: float) -> bool:
        first_sum = False
        if pair not in self.pair_profits.keys():
            # 第一次把所有的pair利润初始化好
            first_sum = True
        
        self.pair_profits[pair] = current_profit
        ...
        logger.info(f'Current total profit:{total_profit:.4f}')
        ...
        if drawdown >= self.TOTAL_DRAWDOWN_THRESHOLD:
            ...
            pair_drawdown = (max_rate - current_rate) / abs(max_rate)
            large_drawdown = (pair_drawdown >= self.PAIR_DRAWDOWN_THRESHOLD)
            low_profit = current_profit < 0
            if large_drawdown or low_profit:
                logger.info(f'Pair {pair} exit because of pair profit large drawdown:{large_drawdown}(drawdown={pair_drawdown:.2%}) or low_profit:{low_profit}(profit={current_profit:.4f})')
                return True
            
        return False

    
class DynamicPosition3mStrategyV9(IStrategy):
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        drawdown_exit = self.total_profit_drawdown_exit.update_and_check_exit(pair, current_profit, trade.max_rate, current_rate)
        if drawdown_exit:
        ...
        if current_profit < profit_drawdown_threshold:
            exit_reason = f'Low absolute profit'
            logger.info(f'{exit_reason}:{current_profit:.2f} < {profit_drawdown_threshold:.2f} for {pair} at {current_time}')
            return exit_reason
