...
class StakePositionManager(IStrategy):
    ...
    atr_entry_base_multiplier = 1.5                 # 进场时的价格ATR倍数，即当前价格超过成本价的这个ATR倍数进场
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        last_order_timestamp = last_order.order_filled_utc
    
        dataframe, _ = self.dp.get_analyzed_dataframe(trade.pair, self.timeframe)
        df_after_last_order = dataframe[dataframe['date'] >= pd.to_datetime(last_order_timestamp)]
        
        if df_after_last_order.empty:
            return False

        count_of_orders = len(filled_orders)
        
        max_profit_spread = 0.02
        drawback_ratio = max(0.7 - (count_of_orders-1) * 0.1, 0.4)
        if trade.is_short:
            max_profit_rate = df_after_last_order.loc[df_after_last_order['low'].idxmin()]['low']
            if max_profit_rate < (1 - max_profit_spread) * open_rate:
            ...
            max_profit_rate = df_after_last_order.loc[df_after_last_order['high'].idxmax()]['high']
            if max_profit_rate > (1 + max_profit_spread) * open_rate:
...
class Dream3V6ManualStrategy(StakePositionManager):
    ...
    ema_trend = 4
    ema_mid_trend = ema_trend * 2
    ema_long_trend = ema_trend * 2
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['ema_plus_atr'] = dataframe['ema_short'] + self.atr_entry_base_multiplier * dataframe['atr']
        dataframe['ema_minus_atr'] = dataframe['ema_short'] - self.atr_entry_base_multiplier * dataframe['atr']
...
class Dream3V6Strategy(Dream3V6ManualStrategy):
        
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe = super().populate_entry_trend(dataframe, metadata)
        
        if self.bidirectional or self.is_long:
            addition_mask = (dataframe['ha_close'] > dataframe['ema_plus_atr']) \
                & (dataframe['ha_close'] > dataframe['ema_short']) \
                & (self.indicator_up_n_periods_mask(dataframe, 'ema_mid', self.ema_mid_trend)) \
                & (self.indicator_up_n_periods_mask(dataframe, 'ema_long', self.ema_long_trend))
        ...
        if self.bidirectional or not self.is_long:
            addition_mask = (dataframe['ha_close'] < dataframe['ema_minus_atr']) \
                        & (dataframe['ha_close'] < dataframe['ema_short']) \
                        & (self.indicator_down_n_periods_mask(dataframe, 'ema_mid', self.ema_mid_trend)) \
                        & (self.indicator_down_n_periods_mask(dataframe, 'ema_long', self.ema_long_trend))
