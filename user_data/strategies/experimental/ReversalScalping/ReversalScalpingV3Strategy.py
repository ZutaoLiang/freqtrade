...
class ReversalScalpingV3Strategy(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe['weak_down_trend'] = 0
        dataframe['enter_long'] = 0
        
        dataframe['weak_up_trend'] = 0
        ...
        if self.down_reveral_to_long:
            ...
            dataframe.loc[
                (
                    ((dataframe['weak_down_trend'] == 0) & (dataframe['weak_down_trend'].shift(1) == 0))
                    & (dataframe['weak_down_trend'].shift(2) == 1)
                    & (dataframe['ha_close'] > dataframe['ha_open'])
                    & ((dataframe['ha_close'] > dataframe['ha_close'].shift(1)) | (dataframe['ha_close'] > dataframe['ha_close'].shift(2)))
            ...
            dataframe.loc[
                (
                    #  & (dataframe['weak_up_trend'].shift(1) == 0)
                    ((dataframe['weak_up_trend'] == 0) & (dataframe['weak_up_trend'].shift(1) == 0))
                    & (dataframe['weak_up_trend'].shift(2) == 1)
                    & (dataframe['ha_close'] < dataframe['ha_open'])
                    #  | (dataframe['ha_close'] < dataframe['ha_close'].shift(2))
                    & ((dataframe['ha_close'] < dataframe['ha_close'].shift(1)) | (dataframe['ha_close'] < dataframe['ha_close'].shift(2)))
