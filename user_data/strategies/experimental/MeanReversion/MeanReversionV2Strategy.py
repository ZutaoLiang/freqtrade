...
class MeanReversionV2Strategy(IStrategy):
    ...
    startup_candle_count = bb_length * 2
    ...
    mean_reversion_change_pct = 0.01
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        bollinger = pta.bbands(close=dataframe['ha_close'], length=self.bb_length, std=self.bb_std, talib=False)
        dataframe['bb_upper'] = bollinger[f'BBU_{self.bb_length}_{self.bb_std}']
        dataframe['bb_middle'] = bollinger[f'BBM_{self.bb_length}_{self.bb_std}']
        dataframe['bb_lower'] = bollinger[f'BBL_{self.bb_length}_{self.bb_std}']
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.is_long:
            dataframe.loc[
                (
                    (dataframe['bb_width'] > self.bb_width_threshold)
                ),
