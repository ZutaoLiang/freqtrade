...
class BreakoutStopLossStrategy(IStrategy):
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        if current_profit > 0.3 * self.trade_leverage:
            return 0.15 * self.trade_leverage
            ...
            return stoploss_from_open(0.04 * self.trade_leverage, current_profit, is_short=trade.is_short, leverage=trade.leverage)
        elif current_profit > 0.05 * self.trade_leverage:
            return stoploss_from_open(0.02 * self.trade_leverage, current_profit, is_short=trade.is_short, leverage=trade.leverage)
        # elif current_profit > 0.01:
            # return stoploss_from_open(0.005 * self.trade_leverage, current_profit, is_short=trade.is_short, leverage=trade.leverage)
        
        # 利润较低，例如不超过5%时，则如果持仓时间长，变成动态止损（始终根据当前价格来计算）
        if current_time - timedelta(hours=24) > trade.open_date_utc:
            return 0.05 * self.trade_leverage
        
        return None
 
    def count_breakout_days(self, row_idx, prices, is_long: bool):
        if row_idx == 0:
            return 0
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe['breakout_long'] = dataframe.index.map(
            lambda x: self.count_breakout_days(x, dataframe['close'], True)
        )
        
        dataframe['breakout_short'] = dataframe.index.map(
            lambda x: self.count_breakout_days(x, dataframe['close'], False)
        )
