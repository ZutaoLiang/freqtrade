...
class TrendConfirmation5mV1(IStrategy):
    timeframe = '5m'
    ...
    addition_min_profit = 0.2
    addition_profit_step = 0.2
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['trend_long'] = 0
        dataframe['trend_short'] = 0
        ...
        dataframe.loc[
            (
                (
                    (dataframe['trend_long'].shift(self.shift_period) == 1) 
                    & (dataframe['ha_close'].shift(self.shift_period) < dataframe['ha_close'])
                    & (dataframe['obv'].shift(self.shift_period) < dataframe['obv'])
                )
                | (
                    (dataframe['trend_long'].shift(self.shift_period+1) == 1) 
                    & (dataframe['ha_close'].shift(self.shift_period+1) < dataframe['ha_close'])
                    & (dataframe['obv'].shift(self.shift_period+1) < dataframe['obv'])
                )
            ), 
        ...
        dataframe.loc[
            (
                (
                    (dataframe['trend_short'].shift(self.shift_period) == 1)
                    & (dataframe['ha_close'].shift(self.shift_period) > dataframe['ha_close'])
                    & (dataframe['obv'].shift(self.shift_period) > dataframe['obv'])
                )
                | (
                    (dataframe['trend_short'].shift(self.shift_period+1) == 1) 
                    & (dataframe['ha_close'].shift(self.shift_period+1) > dataframe['ha_close'])
                    & (dataframe['obv'].shift(self.shift_period+1) > dataframe['obv'])
                )
            ), 
        ...
        **kwargs,
        ...
        if addition_signal:
            if min_stake <= addition_stake <= max_stake:
                logger.info(f'Position addition #{count_of_orders+1} for {trade.pair} with estimated new_profit:{new_open_profit*leverage:.2%}({new_open_profit:.2%}) and stake amount {addition_stake:.5f}, '
                            f'current_profit:{current_profit:.2%}, current_rate:{current_rate:.5f}, new_open_rate:{new_open_rate:.5f} at {current_time}')
                return (addition_stake / leverage, f'entry-addition')
            else:
                logger.warning(f'Skip position addition for {trade.pair} with estimated new_profit:{new_open_profit*leverage:.2%}({new_open_profit:.2%}) and stake amount {addition_stake:.5f} is out of range'
