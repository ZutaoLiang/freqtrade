...
class TrendBreakoutV1(IStrategy):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.profit_drawdown_hours = self.get_config("profit_drawdown_hours", 0) # <= 0 is disabled
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        if max_profit_abs is None:
            ...
            logger.warning(f'{trade.pair} reach new max profit in {open_hours} hours: from {max_profit_abs:.4f} to {total_profit_abs:.4f}, current_rate:{current_rate:.5f} at {current_time}')
            max_profit_abs = total_profit_abs
            trade.set_custom_data(self.MAX_PROFIT_ABS, max_profit_abs)
        ...
        if self.profit_drawdown_enabled:
            if self.profit_drawdown_hours <= 0 or open_hours > self.profit_drawdown_hours:
