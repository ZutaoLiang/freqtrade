...
class ManualStrongTrendV1(IStrategy):
    timeframe = '15m'
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.trailing_stop = self.get_config("trailing_stop", True) # add
        self.use_custom_stoploss = self.get_config("use_custom_stoploss", False)    # add
        
        self.ema_close_length = self.get_config("ema_close_length", 18) # add
        self.enable_ema_exit = self.get_config("enable_ema_exit", True) # add
        ...
        self.reduction_stoploss_ratio = self.get_config("reduction_stoploss_ratio", -1.0)
        
        self.profit_drawdown_enabled = self.get_config("profit_drawdown_enabled", False)    # add
        ...
        self.long_time_low_profit_enabled = self.get_config("long_time_low_profit_enabled", False)  # add
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        try:
            ...
            # ema
            dataframe['ema'] = self.calc_ma(close=dataframe['close'], length=self.ema_close_length)
            ...
            if self.backtesting_mode:
                dataframe = self._calculate_slopes_row_by_row(dataframe)
                logger.info(f"{metadata['pair']} completed")
            else:
                dataframe = self._calculate_slopes_original(dataframe)
            
            return dataframe
        except Exception as e:
            return dataframe
        
    def _calculate_slopes_original(self, dataframe: DataFrame) -> DataFrame:
        dataframe['smooth_close'] = savgol_filter(
            dataframe['ha_close_ma'], 
            window_length=self.close_smooth_window, 
            polyorder=self.close_smooth_polyorder, 
            mode='nearest'
        )
        
        peaks, troughs = self.find_peaks_and_troughs(dataframe['smooth_close'])
        
        filtered_peaks, filtered_troughs = self.filter_extremes(
            peaks, troughs, dataframe['smooth_close'], dataframe.index, 
            distance_threshold=self.extreme_distance_threshold, 
            ratio_threshold=self.extreme_ratio_threshold
        )
    ...
    def _calculate_slopes_row_by_row(self, dataframe: DataFrame) -> DataFrame:
        dataframe['smooth_close'] = np.nan
        dataframe['peak'] = np.nan
        dataframe['trough'] = np.nan
        dataframe['fitted_line'] = np.nan
        dataframe['slope'] = np.nan
        dataframe['relative_slope'] = np.nan
        dataframe['segment_length'] = np.nan
        
        min_required_rows = max(self.close_smooth_window, 20)
        
        for i in range(len(dataframe)):
            if i < min_required_rows:
                continue
            
            sub_df = dataframe.iloc[:i+1].copy()
            
            sub_df['smooth_close'] = savgol_filter(
                sub_df['ha_close_ma'].ffill(), 
                window_length=min(self.close_smooth_window, len(sub_df)), 
                polyorder=self.close_smooth_polyorder, 
                mode='nearest'
            )
            
            peaks, troughs = self.find_peaks_and_troughs(sub_df['smooth_close'])
            
            filtered_peaks, filtered_troughs = self.filter_extremes(
                peaks, troughs, sub_df['smooth_close'], sub_df.index,
                distance_threshold=self.extreme_distance_threshold,
                ratio_threshold=self.extreme_ratio_threshold
            )
            
            sub_df['peak'] = np.nan
            sub_df.loc[filtered_peaks, 'peak'] = sub_df.loc[filtered_peaks, 'smooth_close']
            sub_df['trough'] = np.nan
            sub_df.loc[filtered_troughs, 'trough'] = sub_df.loc[filtered_troughs, 'smooth_close']
            
            fit_results = self.fit_linear_regression(sub_df, filtered_peaks, filtered_troughs)
            
            if i in fit_results.index and not fit_results.loc[i].isna().all():
                dataframe.loc[i, 'smooth_close'] = sub_df.loc[i, 'smooth_close']
                dataframe.loc[i, 'peak'] = sub_df.loc[i, 'peak']
                dataframe.loc[i, 'trough'] = sub_df.loc[i, 'trough']
                dataframe.loc[i, 'fitted_line'] = fit_results.loc[i, 'fitted_line']
                dataframe.loc[i, 'segment_length'] = fit_results.loc[i, 'segment_length']
                                
                if pd.isna(dataframe.loc[i, 'slope']):
                    dataframe.loc[i, 'slope'] = fit_results.loc[i, 'slope']
                    dataframe.loc[i, 'relative_slope'] = fit_results.loc[i, 'relative_slope']
        
        dataframe[['slope', 'relative_slope', 'fitted_line', 'segment_length']] = \
            dataframe[['slope', 'relative_slope', 'fitted_line', 'segment_length']].ffill()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['long_signal'] = 0
        dataframe['short_signal'] = 0
        ...
        try:
            ...
            if not self.backtesting_mode:
                if dataframe.iloc[-1]['long_signal'] == 1:
                    self.feishu_notification.send_message(f"{datetime.now()} {metadata['pair']} - Long signal")
                elif dataframe.iloc[-1]['short_signal'] == 1:
                    self.feishu_notification.send_message(f"{datetime.now()} {metadata['pair']} - Short signal")
