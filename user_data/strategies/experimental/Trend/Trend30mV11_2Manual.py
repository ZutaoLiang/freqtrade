...
class Trend30mV11_2Manual(IStrategy):
    timeframe = '30m'
        ...
        **kwargs,
        ...
        if enough_profit:
            if is_short: # and last_candle['enter_short'] == 1
                addition_signal = (current_rate < last_entry_price * (1 + factor * self.addition_profit_step)) | (last_open_hours > 3 and current_rate < (trade.min_rate * 1.15))
            elif not is_short: # and last_candle['enter_long'] == 1
                addition_signal = (current_rate > last_entry_price * (1 + factor * self.addition_profit_step)) | (last_open_hours > 3 and current_rate > (trade.max_rate * 0.85))
