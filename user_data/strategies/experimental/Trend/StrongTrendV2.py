...
class StrongTrendV2(IStrategy):
    timeframe = '15m'
    trade_leverage = IntParameter(1, 10, default=3, space='buy')
    ...
    base_trailing_stop = 0.1
    base_trailing_stop_offset = 0.05
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.profit_drawdown_activation = self.config.get("profit_drawdown_activation", 1)
        self.profit_drawdown_ratio = self.config.get("profit_drawdown_ratio", 0.4)
        
        self.fee = self.get_config("fee", 0.0005)
        self.long_time_low_profit_hours = self.get_config("long_time_low_profit_hours", 4)
        ...
        self.slope_length = self.get_config("slope_length", 3)
        self.profit_info_log = self.config.get("profit_info_log", False)
        
    def get_config(self, key: str, default: float) -> float:
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        filtered_peaks, filtered_troughs = self.filter_extremes(peaks, troughs, dataframe['smooth_close'], dataframe.index, distance_threshold=3, ratio_threshold=0.01)
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        #         (dataframe['cci'] > self.cci_threshold)
        #     ),
        #     ['exit_short', 'exit_tag']] = (1, 'exit_cci')
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        if max_profit_abs > self.profit_drawdown_activation:
            drawdown_profit_threshold = max_profit_abs * (1 - self.profit_drawdown_ratio)
            if total_profit_abs < drawdown_profit_threshold:
                exit_reason = f'Profit drawdown'
                logger.warning(f'{exit_reason} for {pair}: total profit {total_profit_abs:.4f} < {drawdown_profit_threshold:.4f}(max_profit_abs:{max_profit_abs:.4f} * 1-drawdown:{self.profit_drawdown_ratio:.2%}), current_rate:{current_rate:.5f}, open_rate:{trade.open_rate:.5f} at {current_time}')
                return exit_reason
