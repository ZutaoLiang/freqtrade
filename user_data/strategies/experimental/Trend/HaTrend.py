...
class HaTrend(IStrategy):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.consecutive_exit_count = self.get_config("consecutive_exit_count", 2)
        ...
        self.cooldown_candles = self.get_config("cooldown_candles", 0)
    ...
    def do_calculate_ha(self, df: DataFrame) -> DataFrame:
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        try:
            # haikinashi
            dataframe = self.do_calculate_ha(dataframe)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        try:
            enter_long_mask = (dataframe['ha_close'] > dataframe['ha_open'])
            dataframe.loc[enter_long_mask, ['enter_long', 'enter_tag']] = (1, 'entry_long')
            
            enter_short_mask = (dataframe['ha_close'] < dataframe['ha_open'])
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        try:
            exit_long_trend_mask = (dataframe['ha_close'] < dataframe['ha_open'])
            dataframe.loc[exit_long_trend_mask, 'exit_long_trend'] = 1
            
            exit_short_trend_mask = (dataframe['ha_close'] > dataframe['ha_open'])
            dataframe.loc[exit_short_trend_mask, 'exit_short_trend'] = 1
        ...
        **kwargs,
        ...
        if _current_profit > self.fee:
            ...
            if len(dataframe) >= consecutive_exit_count:
                if trade.is_short:
                    if dataframe['exit_short_trend'].iloc[-consecutive_exit_count:].sum() == consecutive_exit_count:
                        return 'exit-short-positive'
                else:
                    if dataframe['exit_long_trend'].iloc[-consecutive_exit_count:].sum() == consecutive_exit_count:
                        return 'exit-long-positive'
            
            # last_candle = dataframe.iloc[-1].squeeze()
            
            # if trade.is_short:
            #     if last_candle['exit_short_trend'] == 1:
            #         return 'exit-short-positive'
            # else:
            #     if last_candle['exit_long_trend'] == 1:
            #         return 'exit-long-positive'
        # else:
        #     n_consecutive = 6
            
        #     if len(dataframe) >= n_consecutive:
        #         if trade.is_short:
        #             if dataframe['exit_short_trend'].iloc[-n_consecutive:].sum() == n_consecutive:
        #                 return 'exit-short-consecutive'
        #         else:
        #             if dataframe['exit_long_trend'].iloc[-n_consecutive:].sum() == n_consecutive:
        #                 return 'exit-long-consecutive'
