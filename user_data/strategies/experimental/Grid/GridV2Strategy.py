...
class StakePositionManager(IStrategy):
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        recent_volatility = dataframe['atr_pct'].iloc[-20:].mean()
        dynamic_roi_pct = self.roi_pct * (1 + recent_volatility / 4)
        
        last_candle = dataframe.iloc[-1].squeeze()
        atr = last_candle['atr']
        
        for pct in np.arange(20 * dynamic_roi_pct, dynamic_roi_pct-0.001, -0.005):
            if _current_profit > pct:
                logger.info(f'Profit {"short" if is_short else "long"}:{_current_profit:.6f} vs {pct:.6f}, open_rate:{open_rate:.6f}, current_rate:{current_rate:.6f}, stoploss_offset:{(1+factor*pct):.4%}, stoploss_rate:{open_rate*(1+factor*pct):.6f} at {current_time}')
                return stoploss_from_absolute(open_rate*(1+factor*(pct)), current_rate, is_short, leverage)

        # if _current_profit > 0.12:
        #     return stoploss_from_absolute(open_rate*(1+factor*0.12), current_rate, is_short, leverage)

        # if _current_profit > 0.1:
        #     return stoploss_from_absolute(open_rate*(1+factor*0.1), current_rate, is_short, leverage)

        # if _current_profit > 0.08:
        #     return stoploss_from_absolute(open_rate*(1+factor*0.08), current_rate, is_short, leverage)
        
        # if _current_profit > 0.06:
        #     return stoploss_from_absolute(open_rate*(1+factor*0.06), current_rate, is_short, leverage)
        
        # if _current_profit > self.roi_pct:
        #     logger.info(f'profit:{_current_profit:.6f} vs {self.roi_pct + 0.0001:.6f}, offset:{(1+factor*(self.roi_pct)):.4%}, current_rate:{current_rate:.6f}, stop_rate:{open_rate*(1+factor*(self.roi_pct)):.6f} at {current_time}')
        #     return stoploss_from_absolute(open_rate*(1+factor*(self.roi_pct)), current_rate, is_short, leverage)
...
class GridV2Strategy(StakePositionManager):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.bidirectional or self.is_long:
            ...
            dataframe.loc[(
                ...
                & (self.indicator_up_n_periods_mask(dataframe, 'ha_close', self.ema_trend))
                ...
                & (self.indicator_up_n_periods_mask(dataframe, 'ema_long', 3 * self.ema_trend))
                & (self.indicator_up_n_periods_mask(dataframe, 'cci', self.ema_trend))
                ), ['enter_long', 'enter_tag']] = (1, 'entry_long')
        
        if self.bidirectional or not self.is_long:
            dataframe.loc[(
                (dataframe['ha_close'] < dataframe['ema_short'])
                & (self.indicator_down_n_periods_mask(dataframe, 'ha_close', self.ema_trend))
                & (self.indicator_down_n_periods_mask(dataframe, 'ema_short', self.ema_trend))
                & (self.indicator_down_n_periods_mask(dataframe, 'ema_mid', self.ema_trend))
                & (self.indicator_down_n_periods_mask(dataframe, 'ema_long', 3 * self.ema_trend))
                & (self.indicator_down_n_periods_mask(dataframe, 'cci', self.ema_trend))
