...
class RangeV1(IStrategy):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.bb_length = self.get_config("bb_length", 20)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        try:
            ...
            # boll band
            bbands = pta.bbands(close=dataframe['ha_close'], length=self.bb_length, std=2.0, mamode='ema', talib=False)
            dataframe['bb_upper'] = bbands[f"BBB_{self.bb_length}_{self.bb_std}"]
            dataframe['bb_bandwidth'] = bbands[f"BBB_{self.bb_length}_{self.bb_std}"]
