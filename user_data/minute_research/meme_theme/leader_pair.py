...
import meme_theme as PT

SEG = PT.SEG


def load():
    D = {b: PT.bars(b, "1D") for b in PT.P + ["BTC"]}; idx = D["DOGE"].index
...
def book(idx, O, C, FR, n, leader, off, regime="btc", filt=None, TF=None):
    ...
    for d in range(1, len(idx)):
        if leader == "DOGE":
            ...
            if m is not None and m.notna().sum() >= 8:
                cur = m.idxmax()
        if cur is None:
            continue
...
if __name__ == "__main__":
    ...
    if "--filters" in sys.argv:
        ...
        TF = None
    for n, leader, filt in itertools.product((20, 50), ("DOGE", "mom7", "mom28"), filters):
        offs = [0] if leader == "DOGE" else range(7)
        res = {s: [stats(book(idx, O, C, FR, n, leader, o, regime, filt, TF), *SEG[s]) for o in offs] for s in segs}
        row = dict(ema=n, leader=leader, filter=filt or "-")
        for s in segs:
