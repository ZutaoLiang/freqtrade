"""Vectorized MTF Research Engine for Orthogonal Alphas (r6_orthogonal_alphas).

Guarantees:
- Strictly causal mapping from HTF to LTF (always shifted by 1 completed bar).
- Robust handling of bool and float dtypes without lookahead.
- Full support for derivatives fields: OI, Funding, Top-Trader L/S, Taker Volume L/S.
- Scheme-C triple-split evaluation:
  ...
  * HOLDOUT: 2026-03-01 to 2026-08-31 (184 days, sacred blind test)
...
class MTFPanels:
    def __init__(self, base_tf: str = "1h"):
        ...
        self.toptrader = None
        if os.path.exists(f"{self.dir}/sum_toptrader_long_short_ratio.npy"):
            self.toptrader = np.load(f"{self.dir}/sum_toptrader_long_short_ratio.npy", mmap_mode="r")

        # Taker Long/Short Volume Ratio
        self.taker_ls = None
        if os.path.exists(f"{self.dir}/sum_taker_long_short_vol_ratio.npy"):
            self.taker_ls = np.load(f"{self.dir}/sum_taker_long_short_vol_ratio.npy", mmap_mode="r")
